...
public class FlutterWebRTCPlugin implements FlutterPlugin, ActivityAware {
  ...
  public void onDetachedFromActivity() {
    ...
    //peerConnectionClose
    String peerConnectionId = methodCallHandler.userPeerConnectionId;
    methodCallHandler.peerConnectionClose(peerConnectionId);

    //streamDispose
    methodCallHandler.mediaStreamRelease(methodCallHandler.userStreamId);
    methodCallHandler.setActivity(null);

    //videoRendererDispose
    int textureId = methodCallHandler.userTextureId;
    FlutterRTCVideoRenderer render = methodCallHandler.renders.get(textureId);
    if (render == null) {
      Log.d(TAG, "FlutterRTCVideoRendererNotFound : " +  "render [" + textureId + "] not found !");
      return;
    }

    render.Dispose();
    methodCallHandler.renders.delete(textureId);

    Log.d(TAG, "Test ================== " + methodCallHandler.userStreamId);
  }
